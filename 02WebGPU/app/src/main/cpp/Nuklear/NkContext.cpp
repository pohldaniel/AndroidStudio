...
void nkInit(float width, float height) {
	...
	nk_init_default(&nkContext.context, nullptr);
...
void nkInitFont(const char* path) {
	...
	if (path) {
        ...
        AssetIO::LoadAsset(path, data, size);
	...
	wgpuQueueWriteTexture(wgpContext.queue, &font_destination, image_pixels, atlas_width * atlas_height * 4u, &font_source, &font_size);
...
void nkDraw(const WGPUCommandEncoder& commandEncoder, const WGPURenderPassDescriptor& renderPassDescriptor) {
	...
	uint32_t vertex_count = nkContext.vertexBuffer.needed / sizeof(nk_vertex);
    nk_vertex* vertices = (nk_vertex*)nkContext.vertexBufferData;
