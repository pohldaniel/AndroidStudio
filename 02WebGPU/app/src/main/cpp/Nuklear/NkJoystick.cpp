...
void virtual_joystick(struct nk_rect dimension, int touch_id, JoystickResult& out) {
	if (nk_begin(&nkContext.context, "virtual_joystick", dimension, NK_WINDOW_NO_INPUT | NK_WINDOW_NO_SCROLLBAR)){
		...
		nk_virtual_joystick(&nkContext.context, joystick_size,touch_id, out);
	}
...
void nk_virtual_joystick(struct nk_context* ctx, float size_px, int touch_id, JoystickResult& out) {
    ...
    struct nk_vec2 touch_pos = nk_vec2(touch_id == -1 ? 0.0f : touchStates[touch_id].touchX, touch_id == -1 ? 0.0f : touchStates[touch_id].touchY);
...
void virtual_rotation_button(struct nk_rect dimension, int touch_id, RotationButtonResult& out) {
	if (nk_begin(&nkContext.context, "nuclear_widget", dimension, NK_WINDOW_NO_INPUT | NK_WINDOW_NO_SCROLLBAR)) {
		float widget_size = std::min(dimension.w, dimension.h);
		nk_layout_row_static(&nkContext.context, widget_size, widget_size, 1);
		nk_virtual_rotation_button(&nkContext.context, widget_size, touch_id, out);
	}
	nk_end(&nkContext.context);
}
