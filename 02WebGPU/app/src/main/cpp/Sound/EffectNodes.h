...
private:
    float m_lastL = 0.0f;
    float m_lastR = 0.0f;
    float m_alpha = 0.05f; // Filter-St?rke (fest oder dynamisch)
...
private:
    ...
    // EINSTELLUNGEN F?R BRUTALEN EFFEKT:
    ...
    // Verz?gerung in Millisekunden (120ms ist perfekt f?r MG-Rhythmen)
    float m_delayMs = 120.0f;
    float m_sampleRate = 44100.0f;

public:
    SpaceDelayEffect(const std::string& id) : AudioNode(std::move(id)) {
        // Buffer f?r bis zu 1 Sekunde Verz?gerung reservieren
    ...
    void process(int16_t* buffer, int32_t numSamples) override {
        ...
        // Sicherheitscheck, damit wir nicht ?ber den Puffer hinauslesen
        ...
        for (int32_t i = 0; i < numSamples; i += 2) {
            ...
            // 3. Den neuen Sound + das alte Echo (mit Feedback) zur?ck in den Speicher schreiben
            ...
            // 5. In den Haupt-Audiopuffer zur?ckschreiben (mit ?bersteuerungsschutz)
...
private:
    ...
    // Parameter
    float m_depth = 0.03f;      // St?rke des Effekts (0.0 bis 1.0)
...
public:
    ...
    void process(int16_t* buffer, int32_t numSamples) override {
        for (int32_t i = 0; i < numSamples; i += 2) {
            ...
            // 2. LFO (Oszillator) f?r die Verz?gerungs-Modulation berechnen
            ...
            // 5. Zur?ckschreiben und Clamping
            buffer[i] = std::clamp(static_cast<int32_t>(outL), -32768, 32767);
            buffer[i + 1] = std::clamp(static_cast<int32_t>(outR), -32768, 32767);

            // Ring-Buffer Position erh?hen
            m_writePos = (m_writePos + 1) % m_delayBufferL.size();
        }
    }
};