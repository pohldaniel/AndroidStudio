...
MeshCube::MeshCube(const Vector3f &position, float width, float height, float depth, bool generateTexels, const std::string &texture, AAssetManager* assetManager) {
    ...
    m_shader = std::make_shared<Shader>("textureES20.vert", "textureES20.frag", assetManager);

    m_modelMatrix = ModelMatrix();
}

MeshCube::MeshCube(const Vector3f &position, float width, float height, float depth, const std::string &texture, AAssetManager* assetManager) : MeshCube(position, width, height, depth, true, texture, assetManager) {}

MeshCube::MeshCube(float width, float height, float depth, const std::string &texture, AAssetManager* assetManager) : MeshCube(Vector3f(0.0f, 0.0f, 0.0f), width, height, depth, true, texture, assetManager) {}
...
void MeshCube::draw(const Camera camera) {
    ...
    m_shader->loadMatrix("projection",camera.getProjectionMatrix());
    m_shader->loadMatrix("modelView",m_modelMatrix.getTransformationMatrix() *  camera.getViewMatrix());
    m_texture->bind(0);
    glBindVertexArray(m_vao);
...
void MeshCube::setRotPos(const Vector3f &axis, float degrees, float dx, float dy, float dz){
    m_modelMatrix.setRotPos(axis, degrees, dx, dy, dz);
}

void MeshCube::setRotXYZPos(const Vector3f &axisX, float degreesX,
...
void MeshCube::rotate(const Vector3f &axis, float degrees){
    m_modelMatrix.rotate(axis, degrees);
}

void MeshCube::translate(float dx, float dy, float dz){
    m_modelMatrix.translate(dx, dy, dz);
}

void MeshCube::scale(float a, float b, float c){
