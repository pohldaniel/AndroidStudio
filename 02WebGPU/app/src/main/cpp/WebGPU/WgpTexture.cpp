...
WGPUTextureFormat WgpTexture::getFormat() const {
    return m_format;
}

uint32_t  WgpTexture::getMipLevelCount() const {
...
static std::array<float, 3> OutImgToXYZ(int i, int j, int face, float edge) {
    float a = 2.0f * static_cast<float>(i) / edge;
    float b = 2.0f * static_cast<float>(j) / edge;
    if (face == 0) {
...
void WgpTexture::FlipVertical(unsigned char* data, uint32_t width, uint32_t bytesPerChannel, uint32_t height) {
    ...
    unsigned char* pSrcRow = nullptr;
    unsigned char* pDestRow = nullptr;
...
void WgpTexture::FlipHorizontal(unsigned char* data, uint32_t width, uint32_t bytesPerChannel, uint32_t height) {
    ...
    unsigned char* pFront = nullptr;
    unsigned char* pBack = nullptr;
